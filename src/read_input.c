...
static int	map_line(t_data **data, char *line, int height)
{
	...
	while (x - 4 < (*data)->map_width)
	{
		...
		else if (line[x] == (*data)->opponent_id || line[x] == (*data)->opponent_id + 32)
			(*data)->map[height][x - 4] = -1;
		else if (line[x] == (*data)->player_id || line[x] == (*data)->player_id + 32)
...
int	read_map(t_data **data)
{
	...
	line = NULL;
	while (get_next_line((*data)->fd, &line) > 0 && ft_strncmp(line, "Plateau", 6) != 0)
		ft_strdel(&line);
	if (!line)
		return (0);
...
int	read_piece(t_data **data)
{
	...
	line = NULL;
	while (get_next_line((*data)->fd, &line) > 0 && ft_strncmp(line, "Piece", 4) != 0)
		ft_strdel(&line);
	if (!line)
		return (0);
	...
	height = -1;
	while (++height < (*data)->piece_height && get_next_line((*data)->fd, &line))
	{
